...
Now try to read the blog you created using endpoint `blog/read` with no `query` or `doc` args. What you would get is what you assume the blog doc, but have a deep look at `cat` attr. What do you get? Rather than the `_id` of the blog_cat, LIMP dynamically extended this attr and set it to the blog_cat doc value so you can easily get the blog_cat name and desc to present in your app. Thanks to our [extns](/tutorial.md#extns) workflow, you can easily extend any attr in your module using any other module doc, simply by having its `_id` set in its value.
