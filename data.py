...
class Data():
	...
	def update(self, env, collection, attrs, extns, modules, docs, doc):
		doc = self.sanitise_attrs(doc)
		return self.driver.update(env=env, collection=collection, attrs=attrs, extns=extns, modules=modules, docs=docs, doc=doc)
	
	@classmethod
	def delete(self, env, collection, attrs, extns, modules, docs, strategy):
		return self.driver.delete(env=env, collection=collection, attrs=attrs, extns=extns, modules=modules, docs=docs, strategy=strategy)

	@classmethod
	def drop(self, env, collection):
		return self.driver.drop(env=env, collection=collection)
