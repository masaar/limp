...
async def run_app(packages, port):
    ...
    if not Config.realm:
        del modules['realm']
    # [DOC] Update modules Config Attr and call config_data method
    ...
    logger.debug(
        'Loaded modules: %s',
        {module: modules[module].attrs for module in modules.keys()},
    )
    logger.debug(
        'Config has attrs: %s',
        {
    ...
    async def http_handler(request: aiohttp.web.Request):
                        ...
                        .encode(
                            {
                                ...
                                'args': {
                                    'code': f'{modules[module].package_name.upper()}_{module.upper()}_INVALID_ATTR'
                                },
                            }
                        )
                        .encode('utf-8'),
                        ...
                        .encode(
                            {
                                ...
                                'msg': f'{str(e)} for \'{request.method}\' request on module \'{modules[module].package_name.upper()}_{module.upper()}\'.',
                                'args': {
                                    'code': f'{modules[module].package_name.upper()}_{module.upper()}_CONVERT_INVALID_ATTR'
                                },
                            }
                        )
                        .encode('utf-8'),
        ...
        if 'x-auth-bearer' in request.headers or 'x-auth-token' in request.headers:
            ...
            try:
                session_results = await modules['session'].read(
            ...
            if not session_results.args.count:
                ...
                session_results = await modules['session'].reauth(
                    skip_events=[Event.PERM],
                    env=env,
                    query=[
                        {
        ...
        results = await modules[module].methods[method](
            ...
            if msg.type == aiohttp.WSMsgType.TEXT:
                ...
                if (
                    ...
                    if env['quota']['counter'] - 1 <= 0:
                        ...
                        env['quota']['counter'] -= 1
                        asyncio.create_task(
                            handle_msg(env=env, modules=modules, msg=msg)
                        )
                    ...
                    if current_time == job['next_time']:
                        ...
                        if job['type'] == 'job':
                            ...
                            if 'auth' in job.keys():
                                ...
                                session_results = modules['session'].auth(
                                    env=Config._sys_env,
                                    query=[
                                        {
                                ...
                                session = Config._jobs_session
                            job_resuls = modules[job['module']].methods[job['method']](
                                env=Config._sys_env,
                                session=session,
                                query=job['query'],
                                doc=job['doc'],
